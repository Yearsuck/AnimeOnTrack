...
pub fn backup_status(state: State<'_, AppState>) -> Result<BackupStatus, String> {
    ...
    Ok(BackupStatus {
        configured: backup::credentials::is_configured(),
...
pub async fn connect_drive(app: AppHandle, state: State<'_, AppState>) -> Result<BackupStatus, String> {
    let cid = backup::credentials::client_id().ok_or("Google credentials not configured")?;
    let secret = backup::credentials::client_secret().ok_or("Google credentials not configured")?;
...
pub async fn auto_backup_if_due(app: AppHandle) -> Result<(), String> {
    if !backup::credentials::is_configured() {
        return Ok(());
    }
    let state = app.state::<AppState>();
